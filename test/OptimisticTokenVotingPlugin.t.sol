...
contract OptimisticTokenVotingPluginTest is AragonTest {
    ...
    function test_ConstructorDisablesInitializers() public {
        ...
        optimisticPlugin.initialize(dao, settings, votingToken, address(taikoL1), taikoBridge, taikoInbox);
    }
    ...
    function test_InitializeSetsTheRightValues() public {
        ...
        optimisticPlugin = OptimisticTokenVotingPlugin(
            createProxyAndCall(
                ...
                abi.encodeCall(
                    ...
                    (dao, settings, votingToken, address(taikoL1), taikoBridge, taikoInbox)
                )
            )
        );
        (
        ...
        assertEq(timelockPeriod, 12 days, "Incorrect timelockPeriod");
        ...
        optimisticPlugin = OptimisticTokenVotingPlugin(
            createProxyAndCall(
                ...
                abi.encodeCall(
                    OptimisticTokenVotingPlugin.initialize,
                    (dao, settings, votingToken, address(taikoL1), newTaikoBridge, taikoInbox)
                )
            )
        );
    ...
    function test_CanExecuteReturnsFalseWhenAlreadyExecuted() public {
        IDAO.Action[] memory actions = new IDAO.Action[](0);
        uint256 proposalId = optimisticPlugin.createProposal("ipfs://", actions, 0, 4 days);
        (,, uint32 timelockPeriod,,,) = optimisticPlugin.governanceSettings();
    ...
    function test_GovernanceSettingsReturnsTheRightValues() public {
        ...
        optimisticPlugin = OptimisticTokenVotingPlugin(
            createProxyAndCall(
                ...
                abi.encodeCall(
                    OptimisticTokenVotingPlugin.initialize,
                    (dao, newSettings, votingToken, address(taikoL1), taikoBridge, taikoInbox)
                )
            )
        );
